...
`run-local.ps1` 启动基础依赖和五个核心 Java 后端，不启动前端。`seed-demo-data.ps1` 通过真实的 Catalog 与 Inventory local API 幂等创建 20 个固定演示 SKU 和库存。复制默认 `.env.example` 后无需手工填写 SKU ID，页面会通过 Catalog batch API 展示这些真实商品。
